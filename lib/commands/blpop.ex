defmodule Commands.BLPop do
  ...
  def execute(client, [_key, timeout], _cache_pid) do
    :timer.apply_after(String.to_integer(timeout) * 100000, __MODULE__, :close_connection, [client])
  end
  ...
  def handle_event(%Event{type: "element_added"} = event) do
    ...
    case RedisCache.pop_left(event.payload.list_key, "1", event.payload.cache_pid) do
      ...
      element ->
        IO.inspect(element, label: "Popped element from list #{event.payload.list_key} for client #{event.payload.client}")
        :gen_tcp.send(
